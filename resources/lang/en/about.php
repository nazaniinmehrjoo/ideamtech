<?php
return [
    'alt_text' => 'Borna Gostar',
    'title' => 'About Borna Gostar',
    'paragraph_1' => 'Since 2006, Novavaran Borna Gostar Parsi, leveraging skilled specialists and investing in research and development, has localized and optimized brick production line equipment. The company provides diverse services in consulting, design, construction, and implementation for establishing brick manufacturing plants and similar products, addressing customer needs from feasibility studies to maintenance.',
    'paragraph_2' => 'Another key strength of the company is providing diverse services in areas such as consulting, design, construction, and implementation for establishing brick manufacturing plants and similar products like glass wool production lines, cement blocks, tiles, and ceramics. From economic and technical feasibility studies, site selection, production line establishment, and optimization to maintenance, Novavaran Borna Gostar Parsian remains committed to supporting industry professionals from design to execution.',
    'list_item_1' => 'Leader in designing brick manufacturing machinery',
    'list_item_2' => 'Innovator in modern technology',
    'list_item_3' => 'Consulting on brick factory construction',
    'list_item_4' => 'Commitment to after-sales services',

    'certificates' => [
        'subtitle' => 'Certificates and Patents',
        'title' => 'Top Certificates',
        'items' => [
            [
                'title' => 'Fan Blade Design and Manufacture',
                'code' => '024554 A/89',
            ],
            [
                'title' => 'Mechanized Device',
                'code' => '029235 A/89',
            ],
            [
                'title' => 'Brick Loading Robot',
                'code' => '024554 A/89',
            ],
        ],
    ],

    'services_section' => [
        'title' => 'Our Stories and Achievements',
        'customer_satisfaction' => [
            'title' => 'Customer Satisfaction',
            'alt_text' => 'Customer Image',
            'description' => 'Customer satisfaction is our priority. BornaGostar’s team provides professional services and continuous support to ensure a reliable and satisfying experience for you.',
            'link_url' => '/our-customers',
        ],
    ],

    'services_section_two' => [
        'subtitle' => 'From Mission and Values to Catalogs and Technical Approvals',
        'title' => 'Borna Gostar Achievements and Values',
        'tabs' => [
            'tab-1' => [
                'title' => 'Catalog',
                'image' => '/assets/images/resource/catalog.png',
                'description' => 'Download our comprehensive catalog to learn more about our products and services. This catalog provides detailed information about our production, technical specifications, and innovative solutions.',
                'button' => true,
                'button_text' => 'Download Catalog',
                'button_link' => '/assets/catalog/catalog.pdf',
            ],
            'tab-2' => [
                'title' => 'Technical Approval',
                'image' => '/assets/images/resource/taeede.jpg',
                'description' => 'Obtaining technical approvals and certificates of completion ensures adherence to all quality and technical standards throughout the project execution stages.',
                'button' => false,
            ],
            'tab-3' => [
                'title' => 'Mission and Values',
                'image' => '/assets/images/main-slider/pokht.jpg',
                'description' => 'Our goal is to revitalize the brick industry in the country and create distinctive products. We believe in honesty, integrity, and respect as guiding principles for our organization, managers, and employees.',
                'button' => false,
            ],
        ],
        'accordions' => [
            [
                'title' => 'Technical Expertise',
                'icon' => 'fa-regular fa-trophy',
                'items' => [
                    'Acquiring technical knowledge of various dryers.',
                    'Acquiring technical knowledge for building industrial robots.',
                    'Designing axial fans with the highest efficiency for tunnel dryers.',
                    'Modeling brick manufacturing factories before construction.',
                    'Providing design, consulting, and optimization for brick production lines.',
                ],
            ],
            [
                'title' => 'Execution Capability',
                'icon' => 'fa-light fa-user-helmet-safety',
                'items' => [
                    'Building advanced dryers with the latest technology.',
                    'Completing several brick production projects in Iran, Azerbaijan, and Iraq.',
                ],
            ],
            [
                'title' => 'Support and Spare Parts',
                'icon' => 'fa-regular fa-user-headset',
                'items' => [
                    'Contracts for maintaining and repairing brick production lines.',
                    'Providing technical support and spare parts for brick production lines.',
                ],
            ],
        ],
    ],
];
